...
import { Category, levelInfo, uid } from "@/lib/storage";
...
export function SessionRoomBoard({ sessionId, categories, localXP, hostUserId }: Props) {
  ...
  const broadcastSignal = (message: Omit<SignalMessage, "from">) => {
    void channelRef.current?.send({
      type: "broadcast",
      event: "signal",
      payload: { ...message, from: participantId },
    });
  };
  ...
  useEffect(() => {
    trackPresence();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [displayName, selectedCategory, cameraActive, localXP, localLevelInfo]);
  ...
  const startCamera = async () => {
    ...
    if (typeof window !== "undefined" && !window.isSecureContext) {
      setCameraError("Camera needs HTTPS, localhost, or 127.0.0.1 to work.");
      return;
    }
